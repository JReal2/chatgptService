...
def init_consumer():
    conf = {'bootstrap.servers': "20.249.89.31:29092",
            'group.id': "chatgpt",
...
def request_to_chatGPT(data):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "You are a business professional. Any question on the forum can be answered as a comment."},
            {"role": "user", "content": f"다음 질문은 게시판에 올라온 질문입니다. \n"
                                        f"---제목 : {title} \n"
                                        f"내용 : {content} \n"
                                        f"---\n"
                                        f" 해당 질문에 비즈니스 전문가로써 답변해주시기 바랍니다."},
        ]
    )
